  ...
  while (file_->readLine(line)) {
    ...
    if      (len > 2 && line1[0] == 'v' && line1[1] == ' ') {
      ...
      if (! readTextureVertex(line1))
        std::cerr << "Invalid texture vertex line " << line << std::endl;
      ...
      if (! readParameterVertex(line1))
        std::cerr << "Invalid parameter vertex line " << line << std::endl;
    }
    else if (len > 2 && line1[0] == 'g' && line1[1] == ' ') {
      line1 = CStrUtil::stripSpaces(line1.substr(2));

      if (! readGroupName(line1))
        std::cerr << "Invalid group name line " << line << std::endl;
    }
      ...
      std::cerr << "Unrecognised line " << line << std::endl;
  }

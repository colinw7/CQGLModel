...
drawLines(CGeom3DRenderer *renderer)
{
  ...
  for ( ; p1 != p2; ++p1) {
    ...
    renderer->drawLine(CIPoint2D(ppoint1.x, ppoint1.y),
                       CIPoint2D(ppoint2.x, ppoint2.y));
...
drawLines(CGeomZBuffer *zbuffer)
{
  ...
  for ( ; p1 != p2; ++p1) {
    ...
    zbuffer->drawOverlayZLine(int(ppoint1.x), int(ppoint1.y),
                              int(ppoint2.x), int(ppoint2.y));
